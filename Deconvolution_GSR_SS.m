...
mask_SS = logical((AtlasSeeds_Big==5) + (AtlasSeeds_Big==15));
% n = 2;
% startInd = 1;
% freqLow = 0.04;
% calMax = 4;
...
for excelRow = [181 183 185 228 232 236]%
    ...
    if exist(fullfile(maskDir,strcat(recDate,'-',mouseName,'-',sessionType,num2str(1),'-dataFluor.mat')),'file')
        ...
        for ii = startInd:10
            ...
            HbT_SS_filter = lowpass(HbT_SS,lowPass,freq);
            r_region_SS_filter = corr(HbT_SS_filter,HbT_SS_pred(3*freq+1:end));
            subplot(2,2,4)
            plot((1:300)/10,HbT_SS_filter,'r')
            hold on
            plot((1:300)/10,HbT_SS_pred(3*freq+1:3*freq+length(HbT_SS_filter)),'Color',[0 0.5 0])
            xlabel('Time(s)')
            ylabel('\Delta\muM')
            legend(strcat(num2str(lowPass),'Hz Low Passed Actual HbT'),'Predicted HbT')
            title(strcat('r = ',num2str(r_region_SS_filter)))
...
[pks,locs,w,p] = findpeaks(HRF_GSR_SS_0p6_median,t,'MinPeakProminence',0.02);
