...
for ii = 1:nVy
    for jj = 1:nVx
        if mask_barrel(ii,jj)
            ...
            [~,S,~]=svd(X);
            h_pixel(ii,jj,:,2)= (X'*S*X+(S(1,1).^2)*.5*eye(length(Calcium_ori))) \ (X'*S*[zeros(3*freq,1); HbT_ori(1:end-3*freq)]);% why add 3s of zeros? Do we need to shift it?
...
plot((1:length(HbT_ori))/freq,HbT_ori,'k')
hold on
plot((1:length(Calcium_ori))/freq,HbT_pred(3*freq+1:3*freq+length(HbT_ori)),'Color',[0 0.5 0])
ylim([-4 4])
xlabel('Time(s)')
ylabel('\Delta\muM')
...
sgtitle('HRF for Each Pixel, Unfiltered, Awake R5M2285 for Second Block for Run 1')
...
figure
subplot(2,2,1)
plot((1:length(HbT_ori))/freq,HbT_barrel,'k')
ylabel('\Delta\muM')
ylim([-4 4])
hold on
yyaxis right
plot((1:length(Calcium_ori))/freq,Calcium_barrel,'m')
legend('HbT','jRGECO1a')
ylim([-5 5])
ylabel('\DeltaF/F%')
xlabel('Time(s)')
...
hold on
plot((1:length(HbT_barrel))/freq,HbT_barrel_pred(3*freq+1:3*freq+length(HbT_ori)),'Color',[0 0.5 0])
ylim([-4,4])
xlabel('Time(s)')
ylabel('\Delta\muM')
legend('Actual HbT','Predicted HbT')
