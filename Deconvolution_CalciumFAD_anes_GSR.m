...
for excelRow = [202 195 204 230 234 240]
    ...
    for n = 1:3
        ...
        for ii = startInd:20
            ...
            sgtitle(strcat('mrf for Barrel Region, ',num2str(freqLow),'-',num2str(freqHigh),', GSR',' lambda = ',num2str(lambda),', ',mouseName,' Run #',num2str(n),', Segment #',num2str(ii)))
...
mrf_GSR_Barrel_0p6_awake =mrf_GSR_Barrel_0p6;
qualifiedNum_awake = qualifiedNum;
totalNum_awake = totalNum;
mrf_GSR_Barrel_0p6_median_awake = median(mrf_GSR_Barrel_0p6_awake);
[pks_awake,locs_awake,w_awake,p_awake] = findpeaks(mrf_GSR_Barrel_0p6_median_awake,t,'MinPeakProminence',2e-9);

load('191030-R5M2285-anes-R5M2286-anes-R5M2288-anes-R6M2460-anes-R6M1-anes-R6M2497-anes-Barrel_mrf.mat')
mrf_GSR_Barrel_0p6_anes =mrf_GSR_Barrel_0p6;
qualifiedNum_anes = qualifiedNum;
totalNum_anes = totalNum;
mrf_GSR_Barrel_0p6_median_anes = median(mrf_GSR_Barrel_0p6_anes);
[pks_anes,locs_anes,w_anes,p_anes] = findpeaks(mrf_GSR_Barrel_0p6_median_anes,t,'MinPeakProminence',2e-9);

figure('units','normalized','outerposition',[0 0 1 1])
subplot(2,2,1)
plot_distribution_prctile(t,mrf_GSR_Barrel_0p6_awake,'Color',[1 0 0])
legend('25%-75%')
xlim([-3 7])
xlabel('Time(s)')
grid on
...
subplot(2,2,2)
plot_distribution_prctile(t,mrf_GSR_Barrel_0p6_anes,'Color',[0 0 1])
legend('25%-75%')
xlim([-3 7])
xlabel('Time(s)')
grid on
