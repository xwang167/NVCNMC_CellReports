...
info.nVx = 128;
info.nVy = 128;
raw_FAD_mice_awake     = nan(128,128,6);
raw_Calcium_mice_awake = nan(128,128,6);
ll = 1;
miceName = [];
for excelRow = excelRows
    ...
    saveName = strcat(recDate,'-',mouseName,'-',sessionType,'_processed.mat');
    load(fullfile(saveDir, saveName),'xform_raw_FAD_mouse','xform_raw_Calcium_mouse')
    raw_FAD_mice_awake(:,:,ll)     = xform_raw_FAD_mouse;
    raw_Calcium_mice_awake(:,:,ll) = xform_raw_Calcium_mouse;
    ll = ll+1;
end
...
% Visualize baseline fluorescence
figure
subplot(2,2,1)
imagesc(raw_Calcium_mice_awake,[0 60000])
colorbar
hold on
...
title('Calcium Awake')
subplot(2,2,2)
imagesc(raw_FAD_mice_awake,[0 80000])
colorbar
hold on
imagesc(xform_WL,'AlphaData',1-mask);
axis image off
...
subplot(2,2,3)
imagesc(raw_Calcium_mice_anes,[0 60000])
colorbar
hold on
imagesc(xform_WL,'AlphaData',1-mask);
axis image off
...
subplot(2,2,4)
imagesc(raw_FAD_mice_anes,[0 80000])
colorbar
hold on
imagesc(xform_WL,'AlphaData',1-mask);
axis image off
...
calcium_baseline_awake = nan(1,44);
calcium_baseline_anes  = nan(1,44);
FAD_baseline_awake     = nan(1,44);
FAD_baseline_anes      = nan(1,44);

raw_Calcium_mice_awake = reshape(raw_Calcium_mice_awake,1,128*128);
raw_Calcium_mice_anes  = reshape(raw_Calcium_mice_anes ,1,128*128);
raw_FAD_mice_awake     = reshape(raw_FAD_mice_awake    ,1,128*128);
raw_FAD_mice_anes      = reshape(raw_FAD_mice_anes     ,1,128*128);

 
mask = AtlasSeeds.*xform_isbrain_mice;
ii = 1;
for region = [3,4,6:25,28,29,31:50]
    calcium_baseline_awake(ii) =  mean(raw_Calcium_mice_awake(mask==region));
    calcium_baseline_anes(ii)  =  mean(raw_Calcium_mice_anes (mask==region));
    FAD_baseline_awake(ii)     =  mean(raw_FAD_mice_awake    (mask==region));
    FAD_baseline_anes(ii)      =  mean(raw_FAD_mice_anes     (mask==region));
    ii = ii+1;
end
