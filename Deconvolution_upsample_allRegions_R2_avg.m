...
for condition = {'awake','anes'}
    for h = {'HRF','MRF'}   
        eval(strcat('R2_',h{1},'_mice_',condition{1},'_allRegions = nan(6,50);'))   
        eval(strcat('R2_',h{1},'_mice_',condition{1},' = zeros(6,1);'))  
    end
end
...
for condition = {'awake','anes'}
    ...
    for excelRow = eval(strcat('excelRows_',condition{1}))
        ...
        sessionType = excelRaw{6}; sessionType = sessionType(3:end-2);
        for h = {'HRF','MRF'}
             eval(strcat('R2_',h{1},'_mouse_',condition{1},'_allRegions = [];'))
        end
        for n = 1:3
            ...
            % cat r,MRF, HRF
            for h = {'HRF','MRF'}
                eval(strcat( 'R2_',h{1},'_mouse_',condition{1},'_allRegions = cat(1,R2_',h{1},'_mouse_',condition{1},'_allRegions,R2_',h{1},');'))
            end    
        end

        for h = {'HRF','MRF'}
            eval(strcat( 'R2_',h{1},'_mouse_',condition{1},'_allRegions = mean(R2_',h{1},'_mouse_',condition{1},'_allRegions);'))
            ...
            if exist(saveName_mouse,'file')
                eval(strcat('save(',char(39),saveName_mouse,char(39),',',...
                    char(39),'R2_',h{1},'_mouse_',condition{1},'_allRegions',char(39),',',...
            ...
            eval(strcat( 'R2_',h{1},'_mice_',condition{1},'_allRegions(mouseInd,:) = R2_',h{1},'_mouse_',condition{1},'_allRegions;'))           
        end        
        mouseInd = mouseInd+1;
    end
    for h = {'HRF','MRF'}
        if exist(saveName,'file')
            eval(strcat('save(',char(39),saveName,char(39),',',...
                char(39),'R2_',h{1},'_mice_',condition{1},'_allRegions',char(39),',',...
                char(39),'-append',char(39),')'))
        else
            eval(strcat('save(',char(39),saveName,char(39),',',...
                char(39),'R2_',h{1},'_mice_',condition{1},'_allRegions',char(39),')'))
...
for condition = {'awake','anes'}
    for h = {'HRF','MRF'}
        for mouseInd = 1:6
            for region = [3,4,6:25,28,29,31:50]
                eval(strcat('temp = squeeze(nanmean(R2_',h{1},'_mice_',condition{1},'_allRegions(mouseInd,region)))*pixelNum(region)/pixelNumTotal;'))
                eval(strcat( 'R2_',h{1},'_mice_',condition{1},'(mouseInd)=R2_',h{1},'_mice_',condition{1},'(mouseInd)+temp;'))
                clear temp
            end
        end
        if exist(saveName,'file')
            eval(strcat('save(',char(39),saveName,char(39),',',...
                char(39),'R2_',h{1},'_mice_',condition{1},char(39),',',...
                char(39),'-append',char(39),')'))
        else
            eval(strcat('save(',char(39),saveName,char(39),',',...
                char(39),'R2_',h{1},'_mice_',condition{1},'_allRegions',char(39),')'))
...
for condition = {'awake','anes'}
    for h = {'HRF','MRF'}        
           eval(strcat('R2_',h{1},'_',condition{1},'_map =  zeros(1,128*128);'))
           for region = [3,4,6:25,28,29,31:50] 
                mask_region = zeros(128,128);
                mask_region(mask == region) = 1;
                mask_region = logical(mask_region);
                eval(strcat('R2_',h{1},'_',condition{1},'_map(mask_region(:))=mean(R2_',h{1},'_mice_',condition{1},'_allRegions(:,region));'))
           end
           eval(strcat('R2_',h{1},'_',condition{1},'_map = reshape(R2_',h{1},'_',condition{1},'_map,128,128);'))
           if exist(saveName,'file')
               eval(strcat('save(',char(39),saveName,char(39),',',...
                   char(39),'R2_',h{1},'_',condition{1},'_map',char(39),',',...
                   char(39),'-append',char(39),')'))
           else
               eval(strcat('save(',char(39),saveName,char(39),',',...
                   char(39),'R2_',h{1},'_',condition{1},'_map',char(39),')'))
...
for condition = {'awake','anes'}
    for h = {'HRF','MRF'}
        ...
        subplot(2,3,5)
        temp = strcat('imagesc(R2_',h{1},'_',condition{1},'_map,',char(39),'AlphaData',char(39),',mask)');
        eval(temp)
        hold on;
        imagesc(xform_WL,'AlphaData',1-mask);
        cb=colorbar;
