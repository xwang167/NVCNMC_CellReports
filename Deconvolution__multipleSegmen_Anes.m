...
HbT =  filterData(HbT,0.01,0.3,samplingRate);
Calcium = filterData(Calcium,0.02,0.3,samplingRate);
...
for ii = 1:10
    ...
    [~,S,~]=svd(X);
    h_region_barrel = (X'*S*X+(S(1,1).^3)*0.000005*eye(length(Calcium_barrel))) \ (X'*S*[zeros(3*freq,1); HbT_barrel(1:end-3*freq)]);% why add 3s of zeros? Do we need to shift it?
    ...
    sgtitle(strcat('HRF for Barrel Region, 0.02-0.3Hz, lambda = 0.000005, Anestehtized R5M2285 Run 2, Segment #',num2str(ii)))
end
